...
to use the CLI tool. Check [detailed instructions for each command](#-common-commands) below.
...
into your coding agent (like [Google Antigravity](https://antigravity.google/)) and try prompts such as:
...
*   Always activate python virtual environment before running `litert` command, to
    avoid conflicts.
*   When `uv` fails to resolve dependencies, try to set below environment variable first:
...
*  When convert a traditional PyTorch model, you need to write a script to wrap it
   with required functions `get_model` and `get_args`. Check the script format
   in `examples/convert_model.py`.
*  LLM conversion only supports HuggingFace models with type AutoModelForCausalLM and
*  Gemma family now.
*  For large models like LLMs, `litert convert` will take large memories and disks,
    and spend multiple minutes. Please make sure you have enough memory and disks, and be patient.
...
*   When benchmark using `--gcp` flag, you need to 1)
    Join the EAP program of [Google AI Edge Portal](https://ai.google.dev/edge/ai-edge-portal);
...
*   `litert clean` will clean all local caches, like model files and binaries,
    ...
    fixing complicated issues, like issues caused by NPU library version mismatch.
...
with `litert-lm`, for example, both `litert lm run` and `litert-lm run`
or `litert lm benchmark` and `litert-lm benchmark` achieve the same results.
