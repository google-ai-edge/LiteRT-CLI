---
id: litert-cli-101
summary: A one-hour LiteRT CLI 101 hands-on codelab, from basic usages to convert and quantize models, to advanced uses like running benchmarks on physical hardware, and further to agent prompts and cloud testing.
authors: LiteRT CLI Team
categories: Edge AI, LiteRT, Mobile ML
tags: workshop, codelab, litert-cli
status: Draft
feedback_link: https://github.com/google-ai-edge/LiteRT-CLI/issues
keywords: category:EdgeAI, docType:Codelab, product:MobileML
layout: paginated
...
Edge AI requires bringing complex neural models directly onto
...
*   **[LiteRT](https://ai.google.dev/edge/litert)** (formerly **TensorFlow Lite
    ...
    optimization.
*   **[LiteRT CLI](https://github.com/google-ai-edge/litert-cli)** integrates
    Google AI Edge stacks into a standalone shell command (`litert`), to steamline
    LiteRT related development workflows, including converting, quantizing,
    compiling, running, benchmarking and visualizing LiteRT (TFLite) models on
    various hardware (CPU / GPU / NPU) across platforms (desktop, mobile, or
    cloud).

--------------------------------------------------------------------------------

## 🔄 Basic Workflows: conversion, quantize & run
Duration: 15:00

In this section, let us execute a complete Edge AI modeling lifecycle with
LiteRT CLI: PyTorch model wrapper ➔ tracing conversion ➔ model
quantization ➔ desktop inference ➔ performance benchmarks.
...
```mermaid
graph TD
    A[PyTorch Model Source] -->|1. Wrap model| B(resnet18.py)
...
We recommend **[uv](https://docs.astral.sh/uv/getting-started/installation/)**
(a next-generation Rust-based Python package manager) for ultra-fast dependency
downloads and dynamic setup, and you can also use standard
...
**corporate proxy / private simple-index credentials blockers:** If build installs fail with `401 Unauthorized` due to private custom index rules, force the lookup to route directly to standard PyPI:
...
graph LR
    ...
    Model -->|Access from any directory| Commands[litert run / benchmark / compile]
```
...
For this part, you need to connect an Android device with **ADB enabled**.
...
graph TD
    ...
    A -->|--accelerator gpu| C(Mobile OpenCL/WebGPU)
    A -->|--accelerator npu| D(JIT NPU)
    A -->|litert compile| E[AOT Compiled Model]
    E -->|--android --npu| F(AOT NPU)
```
...
The CLI automatically pushes the model to target device, executes the inference
loop, and pipes outputs back!

--------------------------------------------------------------------------------

### 🎮 Target B: Mobile GPU

Offload heavy workoads dynamically to the GPU using OpenCL or WebGPU:
...
*   **Warning**: Dynamic graph builds inside on-device runtime suffer from
...
## 🧠 Advanced Topics: LLMs & Speech Recognition
Duration: 10:00
...
# 3. Generation once
litert lm run models/qwen_w4/model.litertlm --prompt "Explain edge ML."

# 4. Interactive chat
litert lm run models/qwen_w4/model.litertlm
```
...
ASR models like Whisper or Parakeet have both encoder module, and decoder module. 
We can benchmark for specific modules using **Signature Keys**:
...
Here are some examples, and add the LiteRT CLI skill
...
and try prompts like below.

<aside class="positive">

### 💡 Convert, quantize, benchmark and report prompt

*"Convert the FP32 model `litert-community/efficientnet_b1` from Hugging Face. Quantize it using the `dynamic_wi8_afp32` dynamic range recipe. Run high-precision benchmarks on both targets using my desktop CPU. Create a markdown table comparing the average latencies and RAM saving percentages, and write the report as a file."*

</aside>

<aside class="negative">

### 💡 NPU compilation and benchmark prompt
