...
def _build_session_request(
  ...
  Args:
    ...
    execution_timeout_secs: Seconds after which the device stops the job;
      None leaves the platform's default.
  ...
  labels = {"tool": "litert-cli", "accelerator": accelerator, "model": model_name}
  if runtime:
...
def run_ddp(
  ...
  Args:
    model_path_str: Path to the LiteRT model file or .litertlm bundle (local
      or gs://).
    ...
    warmup_runs: Number of warmup iterations before benchmarking. For a
      bundle, the leading iterations left out of the printed medians.
    ...
    timeout: Seconds to wait for the session; None means max_secs (the
      execution timeout for a bundle) times the number of devices plus
    ...
    num_iterations: Prefill and decode cycles of a bundle's benchmark, in
      one process.
  """
  if accelerator == "npu":
