...
We support installation using either **`uv`** (recommended for ultra-fast dependency resolution) or standard **`pip`** within a virtual environment.
...
Check more comprehensive usage examples under the `test_scripts/` directory (e.g., `test_scripts/models/efficientnet.sh`). You can run the CLI via `uv run`:
